...
namespace CapaDatos
{
    public class CD_CargaManifiesto
    {
        public static CD_CargaManifiesto _instancia = null;
        private CD_CargaManifiesto()
        {

        }

        public static CD_CargaManifiesto Instancia
        {
            get
            {
                if (_instancia == null)
                {
                    _instancia = new CD_CargaManifiesto();
                }
                return _instancia;
            }
        }
        ...
        public List<tbSubirManifiesto> DetalleManifiesto(string ruc)
        {
            List<tbSubirManifiesto> listarSolicitud = new List<tbSubirManifiesto>();
            StringBuilder sbSol = new StringBuilder();
            string query = string.Empty;
            try
            {
                sbSol.Append("SELECT ficru2 AS RUC,FICNO5 AS RAZONSOCIAL FROM FICAR7 where FICRU2 = '"+ruc+"'");
                ...
                using (iDB2Connection oConexion = new iDB2Connection(ConexionDB2.CadenaConexion))
                {
                    ...
                    while (dr.Read())
                    {
                        tbSubirManifiesto oSolicitud = new tbSubirManifiesto();
