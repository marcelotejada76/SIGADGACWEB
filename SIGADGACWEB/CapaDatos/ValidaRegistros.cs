...
namespace CapaDatos
{
    class ValidaRegistros
    {
        ...
        //VALIDA MOVIMIENTOS peso
        public static List<CamposUceo> TablaMensajesPeso(string fechaProcesoI, string fechaProcesoF, string Aeropuerto)
        {
            List<CamposUceo> lstUceo = new List<CamposUceo>();
            string query = "";
            //VALIDAPESO

            query = "SELECT OPMFEC , OPMCAL , OPMRE2 ,OPMORI, OPMDES FROM OPMAR1 WHERE OPMPE1 = 0  AND " +
            "OPMORI<>'ZZZZ' AND OPMDES<>'ZZZZ' AND OPMSOB <> 'S' AND OPMFLI <> 'M' AND OPMAE2 = " + "'" + Aeropuerto + "'" + " AND OPMFAC='S' " +
            "AND OPMSTO >= " + "'" + fechaProcesoI + "'" + " AND OPMSTO <= " + "'" + fechaProcesoF + "'" + "";
        ...
        //VALIDA MOVIMIENTOS distancia
        public static List<CamposUceo> TablaMensajesDistancia(string fechaProcesoI, string fechaProcesoF, string Aeropuerto)
        {
            List<CamposUceo> lstUceo = new List<CamposUceo>();
            //VALIDAPESO
            string query = "";

            query = "SELECT OPMFEC , OPMCAL , OPMRE2 , OPMORI,OPMDES FROM OPMAR1 WHERE OPMDI1 = 0  AND OPMAE2 = " + "'" + Aeropuerto + "'" + "  AND OPMSOB <> 'S' AND OPMFLI <> 'M' AND OPMFAC='S' AND OPMORI <>'ZZZZ' AND OPMDES <>'ZZZZ' " +
              "  AND OPMSTO  >=" + "'" + fechaProcesoI + "'" + "AND OPMSTO  <=" + "'" + fechaProcesoF + "'" + " ";
        ...
        //registros sin validar
        public static List<CamposUceo> TablaMensajesRegistrosSinValidar(string fechaProcesoI, string fechaProcesoF, string Aeropuerto)
        {
            List<CamposUceo> lstUceo = new List<CamposUceo>();
            //VALIDAPESO
            string query = "";

            query = "SELECT OPMFEC , OPMCAL , OPMRE2 ,OPMORI,OPMDES FROM OPMAR1 " +
                   "WHERE OPMFAC =' '  AND OPMSOB <> 'S' AND OPMFLI <>'M' AND OPMAE2 = " + "'" + Aeropuerto + "'" + "  AND OPMORI<>'ZZZZ' AND OPMDES<>'ZZZZ' AND OPMFAC=' '  " +
                   "AND OPMSTO  >=" + "'" + fechaProcesoI + "'" + " AND OPMSTO  <=" + "'" + fechaProcesoF + "'" + "";
        ...
        //recupera el oid del aeropuerto
        public static Int32 OidAeropuerto(string Aeropuerto, Int32 OidAto)
        {

            string query = "select OIDOI2 from OIDAR2 WHERE OIDCO3= '" + Aeropuerto + "'";
            ...
            try
            {
                ...
                {
                    CamposUceo oUceo = new CamposUceo();
                    OidAto = Convert.ToInt32(dr["OIDOI2"].ToString().Trim());
        ...
        //cuenta numero de registros de la tabla mensajes
        public static int TablaMensajesNumregistros(string fechaProceso, int NumReg, string Aeropuerto)

        {
            ...
            string query = "SELECT COUNT(OPMFEC) as CANTIDAD FROM OPMAR1 " +
                   "WHERE OPMSOB <> 'S' AND OPMORI<>'ZZZZ' AND OPMDES<>'ZZZZ'  and OPMFLI <>'M'  AND OPMFAC='S' AND OPMFEC  =" + "'" + fechaProceso + "'" + " AND OPMAE2= '" + Aeropuerto + "'";
        ...
        //carga movimientos
        //MOVIMIENTOS AEREOS formato p550
        public static List<CamposUceo> Movimientos(string FechaInicial, string FechaFinal, Int32 OidAto)
        {
            List<CamposUceo> lstUceo = new List<CamposUceo>();
            string query = "select OIDOI7,	OIDOI8,	OIDOI9,	OIDO01,	OIDO02,	OIDFE6,	OIDHO1,	OIDOPE,	OIDUS6,	OIDFE7,OIDO03,OIDFAC,OIDFA1,OIDOR2,	OIDDE1," +
                "OIDO04,	OIDO05,	OIDTOT,	OIDO06,	OIDTIP,	OIDNUM,OIDO07,OIDRUT, OIDOB2, OIDAUT,OIDFA2,OIDO02,OIDPIS from OIDAR3 WHERE OIDOPE<>'SOBREVUELO' " +
                "AND  OIDF06 >= '" + FechaInicial + "' and OIDO02 = " + OidAto + " AND  OIDF06 <= '" + FechaFinal + "'";

            //string query = "select OIDOI7,	OIDOI8,	OIDOI9,	OIDO01,	OIDO02,	OIDFE6,	OIDHO1,	OIDOPE,	OIDUS6,	OIDFE7,OIDO03,OIDFAC,OIDFA1,OIDOR2,	OIDDE1," +
            //    "OIDO04,	OIDO05,	OIDTOT,	OIDO06,	OIDTIP,	OIDNUM,OIDO07,OIDRUT, OIDOB2, OIDAUT,OIDFA2,OIDO02,OIDPIS from OIDAR3 WHERE OIDOPE<>'SOBREVUELO'  and OIDO02 = " + OidAto + "";
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    oUceo.OIDAEROPUERTO = Convert.ToInt32(dr["OIDO02"].ToString().Trim());
                    ...
                    oUceo.AUTORIZACION = dr["OIDAUT"].ToString();
                    oUceo.FACTURAR = dr["OIDFA2"].ToString();
                    oUceo.PISTA = Convert.ToInt16(dr["OIDPIS"]);
