...
function abrirArchivo(fileName) {
    ...
    if (nombreArchivo.trim().length > 0 && opathArchivo.trim().length > 0) {
        ...
        if (_extensionArchivo.toLowerCase() == "pdf") {
            // MOSTRANDO EL LOADING MIENTRAS SE PROCESA
            $("#loadingBuscar").css("display", "flex");

            // CONSTRUIMOS LA URL CON LOS PARÁMETROS PARA EL VISOR DE PDF
            // #navpanes=1: Fuerza la visualización de la barra lateral (miniaturas)
            // #view=FitH: Ajusta la página al ancho del contenedor
            var urlBase = $.MisUrls.url._VisualizarDepositos;
            var texto = urlBase + "?nombreArchivo=" + encodeURIComponent(nombreArchivo) +
                "&direccion=" + encodeURIComponent(opathArchivo) +
                "#navpanes=1&view=FitH";

            setTimeout(function () {
                $("#iframeCetificado").attr("src", texto);
                $("#loadingBuscar").css("display", "none");
            }, 1000); // Reduje el tiempo a 1s para que se sienta más rápido
        }
        else {
            descargarArchivo(nombreArchivo, opathArchivo);
        }
