...
{
    public class DescargaArchivosAftnController : Controller
    {
        private readonly string rutaProcesados = @"\\172.20.19.55\ProcesarAftnIndra\PROCESADOS";
        

        public ActionResult Index(string Nombre = "")
        {
            return ListadoDescargaArchivosAftn(Nombre);
        }

        [HttpGet]
        public ActionResult ListadoArchivosAftn(string Nombre = "")
        {
            return ListadoDescargaArchivosAftn(Nombre);
        }

        [HttpPost]
        public ActionResult ListadoArchivosAftn(string Nombre, HttpPostedFileBase dummy = null)
        {
            return ListadoDescargaArchivosAftn(Nombre);
        }

        [HttpGet]
        public ActionResult ListadoDescargaArchivosAftn(string Nombre = "")
        {
            if (Session["Usuario"] == null)
                return RedirectToAction("login", "Login");

            List<tbDocumentosDescarga> listado = ObtenerArchivosProcesados(Nombre);
            return View("ListadoDescargaArchivosAftn", listado);
        }

        [HttpPost]
        public ActionResult ListadoDescargaArchivosAftn(string Nombre, HttpPostedFileBase dummy = null)
        {
            if (Session["Usuario"] == null)
                return RedirectToAction("login", "Login");
